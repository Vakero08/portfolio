...
export default function CardHome({ project, index }: { project: any, index: any }) {
    ...
    return (
        <div className="md:grid  md:grid-cols-8 md:gap-16 ">
            <div className={`mb-4 overflow-hidden rounded-lg md:col-span-4 h-96 ${index === 1 ? "order-2" : "order-1"} hidden md:block`}>
            ...
            <div className={`md:flex-col md:col-span-4 my-auto ${index === 1 ? "order-1" : "order-2"}`}>
                <div className="mb-4 flex flex-wrap items-baseline gap-3">
                    <h3 className="text-2xl font-bold text-black dark:text-white">{t(`${project.key}.titulo`)}
                    </h3>
                ...
                <p className="mb-8 text-gray-600 dark:text-gray-400">
                    <b>{t(`${project.key}.subtitulo`)} </b>
                    {t(`${project.key}.descripcion`)}
                </p>
                <Link
                    href={`/case${index + 1}`}
                    className="inline-flex items-center gap-2 sm:w-fit w-full justify-center rounded-lg bg-primary px-6 py-3 font-medium text-white transition-colors hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600"
                >
                    {tCard('cta')}
                    <ChevronRight />
                </Link>
